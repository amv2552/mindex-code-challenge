...
public class ChallengeApplicationTests {
	...
	private final String RINGO_STARR_IS = "03aa1462-ffa9-4978-901b-7c001562cf6f";
	...
	public void testJohnLennonReportingStructure() {
		...
		assertEqauls(4, structure.getNumberOfReports());
		asserEquals("John", structure.getEmployee().getFirstName());
	...
	public void testRingoStarrReportingStructure() {
		ReportingStructure structure = employeeService.reportStruct(RINGO_STARR_IS);

		// checks for the number of reports as well as its the correct employee
		assertEqauls(2, structure.getNumberOfReports());
		asserEquals("Ringo", structure.getEmployee().getFirstName());
	}
	...
	public void testPeteBestReportingStructure() {
		...
		// checks for the number of reports as well as its the correct employee
		assertEqauls(0, structure.getNumberOfReports());
		asserEquals("Pete", structure.getEmployee().getFirstName());
	}

	// TASK 2 TESTS

	// checls the compensation exists and saves effectively
	@Test
	public void testCreateCompensationPaulMcMartney() {
		Compesantion comp = employeeService.createComp(
				PAUL_MCCARTNEY_ID,
				200000,
				"01-01-2024"
		);
		...
		assertNotNull(comp);
		assertEquals(PAUL_MCCARTNEY_ID, comp.getEmployee());
	...
	public void testReadCompensationPaulMcMartney() {
		Compensation comp = compensationRepository.findByEmpId(PAUL_MCCARTNEY_ID);
		assertEquals(200000, comp.getSalary());
		assertEquals(200000, comp.getSalary());
		assertEquals("01-01-2024", comp.getEffectiveDate);
	}
